...
function transformDeleteDelete(
  ...
  if (opA.position >= opB.position + opB.length) {
    ...
    // opA is completely overwritten by opB
    return null;
  } else if (
    opA.position <= opB.position &&
    opA.position + opA.length >= opB.position + opB.length
...
function processClientQueue(
  ...
  // sort operations by sourceRevision then batch order
  clientInfo.operationQueue.sort((a, b) => {
    if (a.sourceRevision !== b.sourceRevision) {
      return a.sourceRevision - b.sourceRevision;
    }
  ...
  // process the first queued operation
  const nextOp = clientInfo.operationQueue.shift()!;
  let transformedOp: Operation | null = nextOp.operation;

  console.log(
    `Processing operation from client ${clientInfo.id} based on revision ${nextOp.sourceRevision} (document revision: ${doc.revision})`,
  );

  // Transform the operation against all operations since its source revision
  for (let i = nextOp.sourceRevision; i < doc.revision; i++) {
    if (!transformedOp) break;
    transformedOp = transform(transformedOp, doc.history[i]);
  }
  ...
  if (transformedOp) {
    ...
    // Broadcast the diff (operation) to all other connected clients
    doc.clients.forEach((info, client) => {
      if (client !== ws) {
        client.send(
          JSON.stringify({
    ...
    console.log(`Operation nullified by transformation; sending ack only`);
    ws.send(
      JSON.stringify({
        type: 'operation-ack',
        revision: doc.revision,
  ...
  // Process any further queued operations
  processClientQueue(doc, clientInfo, ws);
}
      ...
      try {
        ...
        switch (type) {
          case 'join-document': {
            ...
            // Send full document state on join
            ws.send(
              JSON.stringify({
                type: 'document-init',
                content: doc.content,
          ...
          case 'update-document': {
            ...
            // Extract operation and batch info
            const { operation, batchId, isLastInBatch, clientRevision } = data;

            console.log(
              `Received operation from client ${clientInfo.id} at revision ${clientRevision}`,
            );

            // If there is a revision mismatch beyond a threshold, update the record
            if (
              clientRevision !== undefined &&
              Math.abs(clientRevision - clientInfo.revision) > 5
            ) {
              console.log(
                `Revision mismatch for client ${clientInfo.id}. Adjusting from ${clientInfo.revision} to ${clientRevision}`,
              );
              clientInfo.revision = clientRevision;
            }

            // Queue the received operation
            clientInfo.operationQueue.push({
              operation,
              sourceRevision: clientRevision || clientInfo.revision,
              batchId,
